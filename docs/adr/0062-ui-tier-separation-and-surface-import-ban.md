...
- `src/ui/features/*/components/**` 有 35 個非測試檔直接 import `@/domains` 或 `@/application`。
...
1. **Surface**（`features/*/pages`、`features/*/components`、`components`）：**不得 import `@/domains`、
   `@/application`、`@/infra`——連型別也不行**。需要 domain 形狀時，一律經由所屬 feature 的 ViewModel 取得。
2. **ViewModel**（`features/*/viewmodels`、`features/*/mappers`）：domain ↔ component 的**唯一橋樑**。
...
4. **Display Labels**（`constants`）：資料值 → 顯示文字的唯一來源。
5. **Presentation Helper**（`utils`）：純格式與樣式函式。

被否決的方案：

- **允許 component 直接使用 domain 型別**（舊規則）。否決理由：這正是 35 個檔案的污染來源；且 component
...
- 需遷移的既有違規：35 個 feature component 檔、18 個 page 檔，以及 `AuthProvider` 的反向依賴。
