...
1. **移除死的 `tx` 執行緒**：permission service、household 讀寫 use case、project snapshot use cases、`useProjectCmds` 不再接受或傳遞 Firestore `Transaction`。UI 層不再 import `firebase/firestore`（登入頁的 `getDoc` 除外，屬 bootstrap 路徑）。
2. **接受活的直接依賴**：application 層繼續直接使用 `runTransaction` 與 Firestore query constraints，不做 `AppTransaction` adapter。理由：
   - ADR-0002 已將 Firebase 定為唯一後端（無 Cloud Functions、無第二儲存後端），adapter 沒有第二個實作，是沒有客戶的假想 seam。
   - `runTransaction` 的 atomicity 編排是 use case 的本職，抽到 repo 只會把交易邏輯切成兩半。
   - 查詢 constraint 的參數物件化只是把 Firestore 詞彙原地重刻一層淺映射，深度為負。
