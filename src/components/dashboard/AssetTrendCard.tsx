...
import { type AssetTrendViewMode } from '@/domains/finance/types';
import { useAssetTrend } from '@/hooks/pages/useAssetTrend';
...
const AssetTrendCard: React.FC<AssetTrendCardProps> = ({ householdId }) => {
  ...
  const chartData = useMemo(() => {
    ...
    let lastDataIndex = -1;
    for (let i = trendData.points.length - 1; i >= 0; i--) {
      const p = trendData.points[i];
      if (
    ...
    return trendData.points.slice(0, lastDataIndex + 1);
  }, [trendData]);
        ...
        <div className="flex items-end justify-between">
          <div className="flex flex-col">
            <span className="font-semibold text-slate-800 tabular-nums">
              實 {formatCurrency(Math.round(actual))}
            </span>
            <span className="text-xs text-slate-400 tabular-nums mb-0.5">
              預 {formatCurrency(Math.round(projected))}
            </span>
          </div>
          <span
