...
export function useRetirementIncomeDialog({
  ...
  const recalculateImport = useCallback(async () => {
    ...
    try {
      const importData = await retirementPlanService.getPlannedIncomeImportData(
        userProfile.householdId,
        selectedCategory,
        importStartDate,
        importEndDate,
      ...
      if (importData) {
        setCalculatedFrom({
          ...importData,
          sampleCount: importSampleCount, // User overrides sample count
        });
        setAmount(Math.round(importData.monthlyAverage * importSampleCount));
      }
    } catch (error) {
