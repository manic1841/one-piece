...
export const accountService = {
  ...
  async recordSnapshot(snapshot: Omit<AccountSnapshot, 'id' | 'createdAt'>): Promise<string> {
  ...
  async getSnapshots(accountId: string, year?: number, month?: number): Promise<AccountSnapshot[]> {
    const q = query(
  ...
  async getLatestSnapshots(householdId: string): Promise<Map<string, AccountSnapshot>> {
    ...
    for (const account of accounts) {
      const snapshots = await this.getSnapshots(account.id);
