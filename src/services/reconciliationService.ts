...
export const reconciliationService = {
    ...
    async getReconciliationReport(householdId: string, year: number, month: number) {
        ...
        for (const account of accounts) {
            const currentMonthSnapshots = await accountService.getBalanceSnapshots(account.id, year, month);
            const prevMonthSnapshots = await accountService.getBalanceSnapshots(account.id, prevYear, prevMonth);

            if (currentMonthSnapshots.length > 0) {
                currentSnapshots.set(account.id, currentMonthSnapshots[0].balance);
            }
            if (prevMonthSnapshots.length > 0) {
                previousSnapshots.set(account.id, prevMonthSnapshots[0].balance);
            }
        }
        ...
        const discrepancyPercentage = previousTotalBalance > 0
            ? (discrepancy / previousTotalBalance) * 100
            : 0;
