    ...
    <div className="space-y-6">
      <Tabs defaultValue="income" className="w-full">
        <TabsList className="grid grid-cols-3 mb-6 bg-slate-100 p-1 rounded-2xl">
          <TabsTrigger value="income" className="rounded-xl font-bold data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-sm">
            <FileText size={16} className="mr-2" /> 損益表
          </TabsTrigger>
          <TabsTrigger value="balance" className="rounded-xl font-bold data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-sm">
            <Wallet size={16} className="mr-2" /> 資產負債表
          </TabsTrigger>
          <TabsTrigger value="cashflow" className="rounded-xl font-bold data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-sm">
        ...
        <TabsContent value="income" className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100">
              ...
              <p className="text-xl font-black text-emerald-700">{formatCurrency(incomeStatement.incomeTotal)}</p>
            </div>
            <div className="bg-rose-50 p-4 rounded-2xl border border-rose-100">
              <p className="text-xs font-bold text-rose-600 uppercase mb-1">總支出</p>
              <p className="text-xl font-black text-rose-700">{formatCurrency(incomeStatement.expenseTotal)}</p>
            </div>
          </div>
          <Card className="rounded-2xl border-slate-100 shadow-sm overflow-hidden">
             <CardHeader className="py-4 bg-slate-50/50">
               <CardTitle className="text-sm font-bold">預計淨損益: {formatCurrency(incomeStatement.netIncome)}</CardTitle>
             </CardHeader>
        ...
        <TabsContent value="balance" className="space-y-4">
           <div className="grid grid-cols-3 gap-4">
            <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100">
              ...
              <p className="text-lg font-black text-blue-700">{formatCurrency(balanceSheet.assets.total)}</p>
            </div>
            <div className="bg-rose-50 p-4 rounded-2xl border border-rose-100">
              <p className="text-xs font-bold text-rose-600 mb-1">負債</p>
              <p className="text-lg font-black text-rose-700">{formatCurrency(balanceSheet.liabilities.total)}</p>
            </div>
             <div className="bg-slate-900 p-4 rounded-2xl">
              <p className="text-xs font-bold text-slate-400 mb-1">淨值</p>
              <p className="text-lg font-black text-white">{formatCurrency(balanceSheet.equity.total)}</p>
            </div>
          </div>
           <Card className="rounded-2xl border-slate-100 shadow-sm overflow-hidden">
             <CardContent className="p-0 max-h-[400px] overflow-y-auto">
        ...
        <TabsContent value="cashflow" className="space-y-4">
           <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
              ...
              <p className="text-lg font-black text-slate-700">{formatCurrency(cashFlow.beginningBalance)}</p>
            </div>
            <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100">
              <p className="text-xs font-bold text-indigo-600 mb-1">期末餘額</p>
              <p className="text-lg font-black text-indigo-700">{formatCurrency(cashFlow.endingBalance)}</p>
            </div>
          </div>
          <Card className="rounded-2xl border-slate-100 shadow-sm overflow-hidden">
             <CardHeader className="py-4 bg-slate-50/50">
                  ...
                  return (
                    <div key={id} className="space-y-1">
                      <div className="flex justify-between px-2 mb-1">
                        ...
                        <span className="text-xs font-mono font-bold">{formatCurrency(group.total)}</span>
                      </div>
