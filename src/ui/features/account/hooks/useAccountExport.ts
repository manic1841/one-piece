...
export function useAccountExport() {
  ...
  const exportToCSV = useCallback(async () => {
    ...
    allData.sort((a, b) => b.date.localeCompare(a.date) || a.accountName.localeCompare(b.accountName));
    ...
    link.setAttribute('download', `account_settlements_${new Date().toISOString().split('T')[0]}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  }, [householdId, auth, fetchAccountsWithSnapshots]);

  const importFromCSV = useCallback(async (file: File): Promise<{ success: number; failed: number; errors: string[] }> => {
    return new Promise((resolve) => {
      ...
      reader.onload = async (e) => {
        ...
        if (!headerLine.includes('日期') || !headerLine.includes('帳戶名稱')) {
          resolve({ success: 0, failed: 0, errors: [`檔案標題不正確。應包含: ${expectedHeaders.join(',')}`] });
          return;
        }
